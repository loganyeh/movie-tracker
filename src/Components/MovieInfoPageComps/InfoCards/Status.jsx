                        ...
                        {statusData.map((status, index) => {
                            return <div key={index} className="h-full w-1/6">
                                        <div className="h-1/2 w-full flex justify-center items-end">
                                            <div className={`h-8/12 w-full flex justify-center items-center text-white text-md ${status.bgColor} rounded-md`}>{status.title}</div>
                                        </div>
                                        <div className="h-auto w-full mt-2 flex justify-center items-center"><span className={`h-full mr-1 text-sm ${status.textColor}`}>{status.users}</span><span className="h-full text-xs">Users</span></div>
                                    </div>
                        })}
